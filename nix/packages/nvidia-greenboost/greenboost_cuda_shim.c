...
static void *gb_get_hook(const char *name)
{
    ...
    /* cudaMemGetInfo not intercepted via dlsym: early callers (device enum)
     * invoke it before a CUDA context exists, causing CUDA_ERROR_INVALID_CONTEXT.
     * libcudart's real cudaMemGetInfo calls cuMemGetInfo_v2 internally, which
     * is hooked above, so inflation still applies. */
