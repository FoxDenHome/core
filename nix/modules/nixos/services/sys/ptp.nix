  ...
  config = lib.mkIf svcConfig.enable {
    ...
    systemd.services.phc2sys = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = [
          "${pkgs.linuxptp}/bin/phc2sys -f /etc/linuxptp/ptp4l.conf -w -s ${svcConfig.interface}"
        ];
      };
    ...
    environment.etc."linuxptp/ptp4l.conf".text = ''
      ...
      refclock_sock_address   /run/refclock.ptp.sock
      ntpshm_segment          0
      ...
      udp_ttl                 1
      udp6_scope              0x0E
      uds_address             /run/ptp4l
      uds_file_mode           0660
      uds_ro_address          /run/ptp4lro
