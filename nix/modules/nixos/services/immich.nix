...
let
  ...
  clipModelName = "ViT-L-14-quickgelu";
        ...
        services.immich = {
          ...
          settings = {
            ffmpeg = {
              ...
              tonemap = "hable";
              accel =
                if config.foxDen.nvidia.enable then
                  "nvenc"
                else if config.foxDen.amdgpu.enable then
                  "vaapi"
                else
                  "none";
              accelDecode = config.foxDen.nvidia.enable || config.foxDen.amdgpu.enable;
            };
            backup = {
              database = {
                enabled = true;
