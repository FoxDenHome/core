    ...
    lib.mkMerge [
      ...
      }).config
      {
        services.factorio = {
          ...
          mods =
            let
              # Needs FACTORIO_AUTH="username=<name>&token=<token>" via nix `impure-env`.
              # NixOS hosts get it from the sops `nix-config` secret; elsewhere put it in
              # the daemon's /etc/nix/nix.conf (ignored from untrusted clients).
              fetchMod =
                name: modInfo:
                derivation {
                  inherit name;
